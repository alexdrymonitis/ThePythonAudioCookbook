...
lcountry = []
# exclude countries that raise a KeyError
excluded_countries = []
# the indexes of these countries have been stored
# in a text file
f = open("excluded_countries.txt", "r")
for line in f:
	line = line.rstrip()
	excluded_countries.append(int(line))
f.close()
# list only countries that don’t raise a KeyError
for i in range(num_countries):
	if i not in excluded_countries:
		lcountry.append(i)
...
def getloc_proc(ndx):
	city = None
	while city is None:
		country = list(cntry.countries)[lcountry[ndx]].name
...
def get_latlng():
	proc=Process(
			target=getloc_proc,
			args=(random.randrange(len(lcountry)),)
		 )
	proc.start()
